...
<head>
    ...
    <title>Optical Geometry Lab | Neon Cone</title>
...
<header class="fixed top-6 left-6 text-left z-50">
    <div class="flex items-center gap-3 mb-2">
        <div class="w-10 h-10 glass-panel rounded-xl flex items-center justify-center">
            <i data-lucide="triangle" class="text-neon w-6 h-6"></i>
        </div>
        <div>
            <h1 class="text-xl md:text-2xl font-black neon-text">Geometric Birth: Cone</h1>
            <p class="text-[10px] text-slate-400 uppercase tracking-widest">Cone Generation Engine</p>
        </div>
    </div>
</header>
...
<script>
    ...
    // Create Right Triangle Geometry (to build cone via rotation)
    const baseWidth = 2;
    const height = 4;
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);               // Cone apex (top)
    shape.lineTo(baseWidth, -height); // Hypotenuse to bottom-outer
    shape.lineTo(0, -height);         // Line to center (base)
    shape.lineTo(0, 0);               // Back to top (axis of rotation)
    ...
    // Center the triangle so the apex is in the middle of the scene
    triangle.position.y = height/2;
    scene.add(triangle);

    // Glowing edges (Triangle perimeter)
    const edgePoints = [
        new THREE.Vector3(0, 0, 0),
        new THREE.Vector3(baseWidth, -height, 0),
    ...
    function animate() {
        ...
        triangle.rotation.y += speed;

        trails.forEach((t, index) => {
            const lag = index * 0.015;
            t.rotation.y = triangle.rotation.y - (lag * speed * 30);
            t.material.opacity = (0.3 / trailsCount) * bloom;
        });

        // Update line color based on bloom intensity
